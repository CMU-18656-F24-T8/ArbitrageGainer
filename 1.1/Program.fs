...
type TradingStrategyEntity(numberOfCryptos: int, minPriceSpread: float, minTransactionProfit: float, maxTransactionValue: float, maxTradingValue: float, initialInvestment: float) =
...
let insertEntity (table: TableClient) (strategy: TradingStrategy) =
    try
        let entity = TradingStrategyEntity(strategy.NumberOfCryptos, strategy.MinPriceSpread, strategy.MinTransactionProfit, strategy.MaxTransactionValue, strategy.MaxTradingValue, strategy.InitialInvestment)
        table.AddEntity(entity) |> ignore
        Success ()
    with
        | :? RequestFailedException as ex when ex.Status = 409 -> Error { Message = "Trading strategy already exists"; Code = 409 }
        | _ -> Error { Message = "Internal server error"; Code = 500 }
    
let updateMaxTradingValue (table: TableClient) (maxTradingValue: MaxTradingValue) =
    try
        match maxTradingValue with
        | { MaxTradingValue = 0.0 } -> Error { Message = "Max trading value cannot be 0 or empty"; Code = 400 }
        | _ ->  
        let entity = table.GetEntity<TradingStrategyEntity>("defaultPartition", "defaultRow").Value
        entity.MaxTradingValue <- maxTradingValue.MaxTradingValue
        table.UpdateEntity(entity, ETag.All, TableUpdateMode.Replace) |> ignore
        Success ()
    with
        | :? RequestFailedException as ex when ex.Status = 404 -> Error { Message = "Trading strategy not found"; Code = 404 }
        | _ -> Error { Message = "Internal server error"; Code = 500 }
...
let setEmail (table: TableClient) (email: Email) =
    try
        ...
        Success ()
    with
        | :? RequestFailedException as ex when ex.Status = 409 -> Error { Message = "Email already exists"; Code = 409 }
        | :? FormatException -> Error { Message = "Invalid email"; Code = 400 }
        | _ -> Error { Message = "Internal server error"; Code = 500 }
...
let getEmail (table: TableClient) =
    try 
        ...
        Success entity
    with 
        | :? RequestFailedException as ex when ex.Status = 404 -> Error { Message = "Email not found"; Code = 404 }
        | _ -> Error { Message = "Internal server error"; Code = 500 }
...
[<EntryPoint>]
let main argv =
    let connectionString = "DefaultEndpointsProtocol=https;AccountName=m2-1;AccountKey=MYUbtQC4VM5dZRJEIAXY2Km7dAPm3etmNn3qt1BUqtsyDpwrF1xlRCiFHt9ue1gygpImoBJBc43yACDb8WgO7A==;TableEndpoint=https://m2-1.table.cosmos.azure.com:443/;"
